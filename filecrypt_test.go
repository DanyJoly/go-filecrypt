...
func TestEncryptOneFile(t *testing.T) {
	doTestEncryptOneFile(t, false)
}

func TestEncryptOneFileWithCompression(t *testing.T) {
	doTestEncryptOneFile(t, true)
}

func doTestEncryptOneFile(t *testing.T, compress bool) {
	cleanup()
	...
	e := EncryptFile(testEncryptOneFileFilepath, cipherFilepath, encrypter, compress)
	if e != nil {
		t.Errorf("Error encrypting file: %v", e)
	}

	e = DecryptFile(cipherFilepath, testOutputDirpath2, decrypter)
	if e != nil {
...
func TestEncryptDir(t *testing.T) {
	doTestEncryptDir(t, false)
}

func TestEncryptFirWithCompression(t *testing.T) {
	doTestEncryptDir(t, true)
}

func doTestEncryptDir(t *testing.T, compress bool) {
	cleanup()

	encrypter, decrypter := createCrypto()
	filename := filepath.Base(testEncryptManyFilesDirepath)
	cipherFilepath := filepath.Join(testOutputDirpath, filename)
	e := EncryptDir(testEncryptManyFilesDirepath, cipherFilepath, encrypter, compress)
	if e != nil {
		t.Errorf("Error encrypting file: %v", e)
	}
	...
	compareDirContent(testEncryptManyFilesDirepath, testOutputDirpath2, t)
